...
%% Define average face size
averageFaceSize = 20; % in cm
...
while keepRunning
    ...
    % Calculate distance to face assuming average face size
    if ~isempty(tracker.Bboxes)
        faceWidthPixels = tracker.Bboxes(3);
        distance = averageFaceSize / faceWidthPixels;
        distanceString = sprintf('Distance to face: %0.2f cm', distance);
        displayFrame = insertText(displayFrame, [10 10], distanceString,...
            'FontSize', 16, 'BoxColor', 'black', 'BoxOpacity', 0.8, 'TextColor', 'white');
    end
